...
static void 
mixed_radix_dit_rec(fft_data *op,fft_data *ip,const fft_object obj, int sgn, int N,int l,int inc)
{
...
void 
fft_exec(fft_object obj,fft_data *inp,fft_data *oup) 
{
    if (obj->lt == 0) {
...
void
twiddle(fft_data *vec,int N, int radix)
{
...
void
longvectorN(fft_data *sig,int N, int *array, int tx)
{
    int L = 1;
    for (int i = 0; i < tx; i++) {
             L = L * array[tx-1-i];
