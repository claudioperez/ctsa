...
static double 
interpolate_linear_sorted_ascending(double *x,double *y, int N, double z)
{
    /*
...
static int
supsmu_(int *n, double *x, double *y, double *w, int *iper, double *span, double *alpha, double *smo, double *sc)
/* ------------------------------------------------------------------ 
 * super-smoother.
 *
...
/* L90: */
    ...
    for (int j = 1; j <= i__1; ++j) {
        ...
        sc[j + (sc_dim1 << 2)] = (1.f - f) * sc[j + sc_dim1 * 3] + f * sc[j + sc_dim1];
        goto L110;
L100:
        f /= spans_1.spans[2] - spans_1.spans[1];
        sc[j + (sc_dim1 << 2)] = (1.f - f) * sc[j + sc_dim1 * 3] + f * sc[j + sc_dim1 * 5];
L110:
        ;
    }
    i__1 = -jper;
